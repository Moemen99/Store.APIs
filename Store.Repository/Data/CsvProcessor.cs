...
namespace Store.Repository.Data
{
    public static class CsvProcessor
    {
        ...
        public static async Task ProcessCsvAsync()
        {
            ...
            var (transactions, errorLog) = ReadTransactions(transactionsLines);
            // Create Csv output
       ...
       private static (List<Transaction>, List<string>) ReadTransactions(List<string> lines)
        {
            ...
            for (int i = 0; i < lines.Count; i++)
            {
                ...
                {
                    string errorMsg = $"Error: Null or empty value found in non-comment field, Line Number: {i + 12}, Good ID: {transactionValues.ElementAtOrDefault(0) ?? "N/A"}, Transaction ID: {transactionValues.ElementAtOrDefault(1) ?? "N/A"}";
